...
# --- 코드 수정 부분 2: 버튼이 session_state를 변경하도록 수정 ---
with col1:
    if st.button('일봉 (3개월)'):
        st.session_state.period = "3mo"
        st.session_state.interval = "1d"
...
# 데이터 불러오기 및 차트 생성
# --- 코드 수정 부분 3: if 'period' in locals() 조건문 삭제 ---
# 항상 session_state에 저장된 값으로 데이터를 불러옵니다.
try:
    # yfinance를 통해 데이터 다운로드
    data = yf.download(ticker,
                       period=st.session_state.period,
                       interval=st.session_state.interval,
                       auto_adjust=True) # auto_adjust=True 추가
